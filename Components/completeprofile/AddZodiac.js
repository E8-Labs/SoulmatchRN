...
const AddZodiac = ({ navigation }) => {
    ...
    const handleSelectZodiac = (itemId) => {
        setError(null)
        setSelectedZodiac(itemId === selectedZodiac ? null : itemId);
    };
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', display: 'flex', justifyContent: 'space-between' }}>
                        {Users.map((item, index) => (
                            <View key={item.id} style={{ height: 113 / 930 * height, width: 113 / 430 * width, marginTop: 20 / 930 * height }}>
                                <TouchableOpacity onPress={() => handleSelectZodiac(item.id)}>
                                    <Image source={selectedZodiac === item.id ? item.selectedZodiacicon : item.zodiacicon} style={{ height: 113 / 930 * height, width: 113 / 430 * width, resizeMode: 'contain' }} />
                                </TouchableOpacity>
                            </View>
                        ))}
                    </View>
